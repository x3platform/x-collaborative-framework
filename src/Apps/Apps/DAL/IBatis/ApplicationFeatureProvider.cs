namespace X3Platform.Apps.DAL.IBatis
{
  ...
  public class ApplicationFeatureProvider : IApplicationFeatureProvider
  {
    ...
    public IList<ApplicationFeatureInfo> GetPaging(int startIndex, int pageSize, DataQuery query, out int rowCount)
    {
      ...
      {
        string applicationId = StringHelper.ToSafeSQL(query.Where["ApplicationId"].ToString());
        ...
        {
          args.Add("WhereClause", " T.ApplicationId = \'" + applicationId + "\' AND T.ParentId = \'" + parentId + "\' ");
        }
      }
      else
      {
      ...
      IList<ApplicationFeatureInfo> list = ibatisMapper.QueryForList<ApplicationFeatureInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_GetPaging", tableName)), args);
    ...
    public DataTable GetApplicationFeatureScope(string applicationId, string authorizationObjectType, string authorizationObjectId)
    {
      ...
      return this.ibatisMapper.QueryForDataTable(StringHelper.ToProcedurePrefix(string.Format("{0}_Scope_GetRelations", tableName)), args);
    }
    #endregion
