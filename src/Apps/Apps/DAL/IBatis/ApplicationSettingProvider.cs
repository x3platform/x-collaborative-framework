  ...
  public class ApplicationSettingProvider : IApplicationSettingProvider
  {
    ...
    public IList<ApplicationSettingInfo> GetPaging(int startIndex, int pageSize, DataQuery query, out int rowCount)
    {
      ...
      if (query.Variables["scence"] == "Query")
      {
        ...
        {
          args.Add("WhereClause", " ApplicationId = '" + applicationId + "' ");
        }
        else
        {
          args.Add("WhereClause", " ApplicationId = '" + applicationId + "' AND T.Code LIKE '%" + searhText + "%' OR T.Text LIKE '%" + searhText + "%' OR T.Value LIKE '%" + searhText + "%' ");
        }
      }
