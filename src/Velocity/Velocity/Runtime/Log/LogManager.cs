...
namespace X3Platform.Velocity.Runtime.Log
{
    using System;
    ...
    public class LogManager
    {
        ...
        public static ILogSystem CreateLogSystem(IRuntimeServices runtimeServices)
        {
            ...
            catch (ApplicationException applicationException)
            {
                String error =
                    string.Format(
                        "PANIC : X3Platform.Velocity cannot find any of the specified or default logging systems in the classpath, or the classpath doesn't contain the necessary classes to support them. Please consult the documentation regarding logging. Exception : {0}",
                        applicationException);
