...
namespace X3Platform.Velocity.Runtime.Resource
{
    using System;
        ...
        public void Initialize(IRuntimeServices rs)
        {
            ...
            if (resourceManagerCacheClassName != null && resourceManagerCacheClassName.Length > 0)
            {
                ...
                if (!(o is ResourceCache))
                {
                    String err =
                        string.Format(
                            "The specified class for ResourceCache ({0}) does not implement X3Platform.Velocity.Runtime.Resource.ResourceCache. Using default ResourceCache implementation.",
                            resourceManagerCacheClassName);
                    runtimeServices.Error(err);
                    o = null;
                }
