...
//
// FileName     :IEntitySnapshotProvider.cs
//
// Description  :
//
...
namespace X3Platform.Entities.DAL.IBatis
{
    ...
    public class EntitySnapshotProvider : IEntitySnapshotProvider
    {
        ...
        public IList<EntitySnapshotInfo> GetPages(int startIndex, int pageSize, string whereClause, string orderBy, out int rowCount)
        {
            ...
            IList<EntitySnapshotInfo> list = this.ibatisMapper.QueryForList<EntitySnapshotInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_GetPages", tableName)), args);
