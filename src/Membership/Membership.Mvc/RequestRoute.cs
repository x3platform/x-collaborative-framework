    ...
    public class RequestRoute : RouteBase
    {
        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            if (virtualPath.IndexOf(prefixUrl) == 0)
            {
                ...
                else if (Regex.IsMatch(friendlyUrl, @"^settings/contact$"))
                {
                    // 设置 - 头像信息
                    routeData.Values.Add("controller", "Settings");
                    routeData.Values.Add("action", "Contact");
                }
                ...
                else if (Regex.IsMatch(friendlyUrl, @"^friends$"))
                {
                    // 设置 - 好友管理
                    routeData.Values.Add("controller", "Friends");
                    routeData.Values.Add("action", "Index");
                }
                else if (Regex.IsMatch(friendlyUrl, @"^friends/detail/([\w+\-]+)$"))
                {
                    // 设置 - 好友管理
                    routeData.Values.Add("controller", "Friends");
                    routeData.Values.Add("action", "Detail");
                    routeData.Values.Add("options", "{\"id\":\"" + Regex.Match(friendlyUrl, @"^friends/detail/([\w+\-]+)$").Groups[1].Value + "\"}");
                }
                else if (Regex.IsMatch(friendlyUrl, @"^friends/invite$"))
                {
                    // 设置 - 好友管理
                    routeData.Values.Add("controller", "Friends");
                    routeData.Values.Add("action", "Invite");
                }
                else if (Regex.IsMatch(friendlyUrl, @"^friends/accept$"))
                {
                    // 设置 - 好友管理
                    routeData.Values.Add("controller", "Friends");
                    routeData.Values.Add("action", "Accept");
                }
                else
                {
                    return null;
