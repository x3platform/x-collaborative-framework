...
namespace X3Platform.Membership.DAL.IBatis
{
    ...
    public class OrganizationProvider : IOrganizationProvider
    {
        ...
        {
            string whereClause = string.Format(" Id IN ( dbo.func_GetCorporationIdByOrganizationId( ##{0}## ) ) ", StringHelper.ToSafeSQL(id));
        ...
        {
            string whereClause = string.Format(" Id IN ( dbo.func_GetDepartmentIdByOrganizationId( ##{0}## , {1} ) ) ", StringHelper.ToSafeSQL(organizationId), level);
...
Id IN (
    SELECT dbo.func_GetCorporationIdByOrganizationId( Id ) FROM tb_Organization WHERE Id IN (
        ...
        public IList<IOrganizationInfo> GetPages(int startIndex, int pageSize, string whereClause, string orderBy, out int rowCount)
        {
            ...
            IList<IOrganizationInfo> list = this.ibatisMapper.QueryForList<IOrganizationInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_GetPages", tableName)), args);
        ...
        public bool IsExist(string id)
        {
            if (string.IsNullOrEmpty(id))
                throw new ArgumentException("实例标识不能为空。");
