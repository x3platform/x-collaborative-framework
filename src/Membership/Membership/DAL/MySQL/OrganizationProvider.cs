...
namespace X3Platform.Membership.DAL.MySQL
{
    ...
    public class OrganizationProvider : IOrganizationProvider
    {
        ...
        /// <param name="keys">标识,多个以逗号分开</param>
        ...
        public IOrganizationInfo FindOne(string id)
        {
            ...
            OrganizationInfo param = ibatisMapper.QueryForObject<OrganizationInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_FindOne", tableName)), args);
        ...
        {
            string whereClause = string.Format(" Id IN ( func_GetCorporationIdByOrganizationId( ##{0}## ) ) ", StringHelper.ToSafeSQL(id));
        ...
        public IList<IOrganizationInfo> FindAll(string whereClause, int length)
        {
            ...
            IList<IOrganizationInfo> list = ibatisMapper.QueryForList<IOrganizationInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_FindAll", tableName)), args);
...
Id IN (
    SELECT OrganizationId FROM tb_Role WHERE Id IN ( SELECT RoleId FROM tb_Account_Role WHERE AccountId = ##{0}## )
        ...
        public IList<IOrganizationInfo> FindCorporationsByAccountId(string accountId)
        {
            string whereClause = string.Format(@" 
Id IN (
    SELECT func_GetCorporationIdByOrganizationId( Id ) FROM tb_Organization WHERE Id IN (
        SELECT OrganizationId FROM tb_Role WHERE Id IN ( SELECT RoleId FROM tb_Account_Role WHERE AccountId = ##{0}## )
)) ", StringHelper.ToSafeSQL(accountId));
        ...
        public IList<IOrganizationInfo> GetPages(int startIndex, int pageSize, string whereClause, string orderBy, out int rowCount)
        {
            ...
            IList<IOrganizationInfo> list = ibatisMapper.QueryForList<IOrganizationInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_GetPages", tableName)), args);
        ...
        private IList<IAccountOrganizationRelationInfo> FindAllRelation(string whereClause)
        {
            ...
            return ibatisMapper.QueryForList<IAccountOrganizationRelationInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_FindAllRelation", tableName)), args);
        }
        #endregion
        ...
        public int ExtendRelation(string accountId, string organizationId, DateTime endDate)
        {
            ...
            ibatisMapper.Update(StringHelper.ToProcedurePrefix(string.Format("{0}_ExtendRelation", tableName)), args);
        ...
        public bool HasDefaultRelation(string accountId)
        {
            ...
            isExist = ((int)ibatisMapper.QueryForObject(StringHelper.ToProcedurePrefix(string.Format("{0}_HasDefaultRelation", tableName)), args) == 0) ? false : true;
        ...
        public int SetNullDefaultRelation(string accountId)
        {
            ...
            ibatisMapper.Update(StringHelper.ToProcedurePrefix(string.Format("{0}_SetNullDefaultRelation", tableName)), args);
        ...
        public int SetDefaultRelation(string accountId, string organizationId)
        {
            try
            {
                ...
                // 2.清空默认关系
                ibatisMapper.Update(StringHelper.ToProcedurePrefix(string.Format("{0}_ResetRelation", tableName)), args);
                // 3.设置 tb_Member 表的默认属性
                ibatisMapper.Update(StringHelper.ToProcedurePrefix(string.Format("{0}_SetDefaultRelation", tableName)), args);
                // 4.设置 IsDefault = 1
                ibatisMapper.Update(StringHelper.ToProcedurePrefix(string.Format("{0}_SetDefaultRelationIsDefault", tableName)), args);
