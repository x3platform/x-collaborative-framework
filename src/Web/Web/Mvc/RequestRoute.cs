    ...
    public class RequestRoute : RouteBase
    {
        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            if (virtualPath.IndexOf(prefixUrl) == 0)
            {
                ...
                // 限制名称空间
                routeData.DataTokens["Namespaces"] = new string[] { "X3Platform.Web.Customizes.Mvc.Controllers" };

                if (!string.IsNullOrEmpty(friendlyUrl))
                {
                    routeData.Values.Add("controller", "CustomizeContent");
                    routeData.Values.Add("action", "GenerateContent");
                    routeData.Values.Add("options", "{\"name\":\"" + friendlyUrl + "\"}");
                }
                else
                {
                    return null;
