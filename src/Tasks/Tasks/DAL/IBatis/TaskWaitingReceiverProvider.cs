namespace X3Platform.Tasks.DAL.IBatis
{
  ...
  /// <summary></summary>
  [DataObject]
  public class TaskWaitingReceiverProvider : ITaskWaitingReceiverProvider
  {
    /// <summary>配置</summary>
    ...
    private string tableName = "tb_TaskWaiting_Receiver";

    /// <summary></summary>
    public TaskWaitingReceiverProvider()
    {
    ...
    /// <returns>返回一个 TaskWaitingReceiverInfo 实例的详细信息</returns>
    public TaskWaitingItemInfo FindOne(string timingTaskId, string receiverId)
    {
      Dictionary<string, object> args = new Dictionary<string, object>();

      args.Add("TaskWaitingId", timingTaskId);
    ...
    /// <returns>返回所有 TaskWaitingReceiverInfo 实例的详细信息</returns>
    ...
    /// <returns>返回一个列表实例<see cref="TaskWaitingReceiverInfo"/></returns>
    public IList<TaskWaitingItemInfo> GetPaging(string receiverId, int startIndex, int pageSize, string whereClause, string orderBy, out int rowCount)
    {
      Dictionary<string, object> args = new Dictionary<string, object>();
      ...
      IList<TaskWaitingItemInfo> list = this.ibatisMapper.QueryForList<TaskWaitingItemInfo>(StringHelper.ToProcedurePrefix(string.Format("{0}_GetPaging", tableName)), args);
    ...
    #region 函数:IsExist(TaskWaitingReceiverInfo param)
    ...
    public bool IsExist(string timingTaskId, string receiverId)
    {
      ...
      args.Add("WhereClause", string.Format(" TaskWaitingId='{0}' and ReceiverId='{1}'",
    ...
    public void SetSent(string receiverId, string taskIds)
    {
      ...
      string whereClause = string.Format(" ReceiverId='{0}' AND TaskWaitingId IN ('{1}') ", receiverId, taskIds.Replace(",", "','"));
