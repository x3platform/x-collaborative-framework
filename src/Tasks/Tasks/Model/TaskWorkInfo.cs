namespace X3Platform.Tasks.Model
{
    ...
    [Serializable]
    public class TaskWorkInfo : IMessageObject, ISerializedObject, ICacheable
    {
        ...
        private IList<TaskWorkReceiverInfo> m_ReceiverGroup = null;

        /// <summary>接收者</summary>
        public IList<TaskWorkReceiverInfo> ReceiverGroup
        {
            get
            {
                if (m_ReceiverGroup == null)
                {
                    m_ReceiverGroup = new List<TaskWorkReceiverInfo>();
                }
        ...
        public virtual string Serializable(bool displayComment, bool displayFriendlyName)
        {
            ...
            foreach (TaskWorkReceiverInfo item in this.ReceiverGroup)
            {
                if (item.Status == 1)
                {
        ...
        public IList<TaskWorkItemInfo> GetTaskWorkItems()
        {
            ...
            foreach (TaskWorkReceiverInfo receiver in this.ReceiverGroup)
            {
