namespace X3Platform.Tasks.BLL
{
    ...
    /// <summary></summary>
    public class TaskWaitingReceiverService : ITaskWaitingReceiverService
    {
        /// <summary>配置</summary>
        private TasksConfiguration configuration = null;

        /// <summary>数据提供器</summary>
        private ITaskWaitingReceiverProvider provider = null;

        private DateTime actionTime = DateTime.Now;

        #region 构造函数:TaskWorkReceiverService()
        /// <summary>构造函数:TaskWorkReceiverService()</summary>
        public TaskWaitingReceiverService()
        {
            ...
            // 创建数据提供器
            this.provider = objectBuilder.GetObject<ITaskWaitingReceiverProvider>(typeof(ITaskWaitingReceiverProvider));
        }
        #endregion
        ...
        {
            return provider.GetPaging(receiverId, startIndex, pageSize, whereClause, orderBy, out rowCount);
        }
        #endregion
