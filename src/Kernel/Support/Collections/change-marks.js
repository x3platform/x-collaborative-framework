...
var ignoreFiles = ['README.md', 'change-marks.js', 'ReadOnlyDictionary.cs', 'SyncDirectionary.cs', '.csproj'],
    ignoreDirectories = ['temp', 'bin', 'obj', 'Properties'];

// 需要修改的标记
var marks = [
{
    before: '[assembly: CLSCompliant(true)]',
    after: '// [assembly: CLSCompliant(true)]'
},
{
    before: 'Wintellect.PowerCollections',
    after: 'X3Platform.Collections'
}];
    ...
    files.forEach(function(file)
    {
        ...
        marks.forEach(function(mark)
        {
            if (options.restore)
            {
                ...
                {
                    changed = true;
                    content = content.replace(new RegExp(mark.after.replace('[','\\[').replace(']','\\]').replace('(','\\(').replace(')','\\)'), 'g'), mark.before);
                }
            }
            else
            {
                ...
                {
                    console.log(mark.before.replace('[','\\[').replace(']','\\]').replace('(','\\(').replace(')','\\)'));

                    changed = true;
                    content = content.replace(new RegExp(mark.before.replace('[','\\[').replace(']','\\]').replace('(','\\(').replace(')','\\)'), 'g'), mark.after);
                }
            }
        });
