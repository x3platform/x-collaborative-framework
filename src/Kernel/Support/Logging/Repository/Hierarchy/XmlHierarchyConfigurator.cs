...
namespace X3Platform.Logging.Repository.Hierarchy
{
    ...
    public class XmlHierarchyConfigurator
    {
                ...
                foreach (XmlNode currentNode in appenderElement.ChildNodes)
                {
                    ...
                    if (currentNode.NodeType == XmlNodeType.Element)
                    {
                        ...
                        if (currentElement.LocalName == APPENDER_REF_TAG)
                        {
                            ...
                            {
                                LogLog.Error(declaringType, "Requesting attachment of appender named [" + refName + "] to appender named [" + appender.Name + "] which does not implement X3Platform.Logging.Core.IAppenderAttachable.");
                            }
                        }
                        else
                        {
        ...
        protected void ParseLogger(XmlElement loggerElement)
        {
            ...
            LogLog.Debug(declaringType, "Retrieving an instance of X3Platform.Logging.Repository.Logger for logger [" + loggerName + "].");
