...
{
	/// <summary>
	/// Initializes the X3Platform.Logging environment using an XML DOM.
	/// </summary>
	/// <remarks>
	/// <para>
	...
	public class XmlHierarchyConfigurator
	{
				...
				foreach (XmlNode currentNode in appenderElement.ChildNodes)
				{
					...
					if (currentNode.NodeType == XmlNodeType.Element) 
					{
						...
						if (currentElement.LocalName == APPENDER_REF_TAG)
						{
							...
							{
								LogLog.Error("XmlHierarchyConfigurator: Requesting attachment of appender named ["+refName+ "] to appender named [" + appender.Name + "] which does not implement X3Platform.Logging.Core.IAppenderAttachable.");
							}
						}
						else
						{
		...
		{
			// Create a new X3Platform.Logging.Logger object from the <logger> element.
			string loggerName = loggerElement.GetAttribute(NAME_ATTR);

			LogLog.Debug("XmlHierarchyConfigurator: Retrieving an instance of X3Platform.Logging.Repository.Logger for logger [" + loggerName + "].");
