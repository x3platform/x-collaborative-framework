...
namespace X3Platform.Logging.Config
{
	...
	public sealed class XmlConfigurator
	{
		...
		static public void Configure(ILoggerRepository repository) 
		{
			...
			try
			{
				...
				if (configElement == null)
				{
                    ...
                    LogLog.Error("XmlConfigurator: Failed to find configuration section 'logging' in the application's .config file. Check your .config file for the <logging> and <configSections> elements. The configuration section should look like: <section name=\"logging\" type=\"X3Platform.Logging.Config.LoggingConfigurationSectionHandler,X3Platform.Support\" />");
				}
				else
				{
			...
			catch(System.Configuration.ConfigurationException confEx)
			{
				...
				else
				{
					...
					string configSectionStr = "<section name=\"logging\" type=\"X3Platform.Logging.Config.LoggingConfigurationSectionHandler," + Assembly.GetExecutingAssembly().FullName + "\" />";
		...
		/// <configSections>
		///		<section name="X3Platform.Logging" type="System.Configuration.IgnoreSectionHandler" />
		///	</configSections>
		/// </code>
		/// </para>
		/// <example>
		...
		static public void Configure(ILoggerRepository repository, Stream configStream)
		{
			...
			else
			{
				...
				if (doc != null)
				{
					...
					{
                        LogLog.Debug("XmlConfigurator: XML configuration does not contain a <logging> element. Configuration Aborted.");
					}
					else if (configNodeList.Count > 1)
					{
                        LogLog.Error("XmlConfigurator: XML configuration contains [" + configNodeList.Count + "] <logging> elements. Only one is allowed. Configuration Aborted.");
					}
					else
					{
