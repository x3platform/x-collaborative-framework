...
using X3Platform.Logging.Repository.Hierarchy;
...
namespace X3Platform.Logging.Core
{
	...
	/// </para>
	/// <para>
	/// This class is used by the wrapper managers (e.g. <see cref="X3Platform.Logging.LogManager"/>)
	...
	/// the <see cref="RepositorySelector"/> property, or by setting the <c>X3Platform.Logging.RepositorySelector</c>
	...
	public sealed class LoggerManager
	{
		...
		/// These are used to shutdown the X3Platform.Logging system as the application exits.
		/// </para>
		/// </remarks>
...
#if NETCF
			s_repositorySelector = new CompactRepositorySelector(typeof(X3Platform.Logging.Repository.Hierarchy.Hierarchy));
#else

			// Look for the RepositorySelector type specified in the AppSettings 'X3Platform.Logging.RepositorySelector'
			string appRepositorySelectorTypeName = SystemInfo.GetAppSetting("X3Platform.Logging.RepositorySelector");
		...
		private static string GetVersionInfo()
		{
			...
			// Build Up message
			sb.Append("X3Platform.Logging assembly [").Append(myAssembly.FullName).Append("]. ");
