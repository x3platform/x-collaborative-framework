...
namespace X3Platform.Logging.Core
{
	...
	public class DefaultRepositorySelector : IRepositorySelector
	{
		...
		/// an appropriate value would be <see cref="X3Platform.Logging.Repository.Hierarchy.Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		/// to create can be overridden by specifying the <see cref="X3Platform.Logging.Config.RepositoryAttribute"/> 
		/// attribute on the <paramref name="repositoryAssembly"/>.
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="X3Platform.Logging.Repository.Hierarchy.Hierarchy"/> 
		...
		/// any <see cref="X3Platform.Logging.Config.ConfiguratorAttribute"/> attributes defined on
		...
		/// <see cref="X3Platform.Logging.Config.RepositoryAttribute"/> attribute on the 
		...
		/// configured using any <see cref="X3Platform.Logging.Config.ConfiguratorAttribute"/> 
		...
		/// Also the <see cref="X3Platform.Logging.Config.RepositoryAttribute"/> attribute on the
		...
		/// </summary>
		/// <param name="assembly">The assembly that has a <see cref="X3Platform.Logging.Config.RepositoryAttribute"/>.</param>
		...
		private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
		{
			...
			try
			{
				...
				else
				{
					...
					{
						LogLog.Error("DefaultRepositorySelector: Assembly [" + assembly + "] has multiple X3Platform.Logging.Config.RepositoryAttribute assembly attributes. Only using first occurrence.");
					}
		...
		/// </summary>
		/// <param name="assembly">The assembly containing <see cref="X3Platform.Logging.Config.ConfiguratorAttribute"/>
		...
		private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
		{
			...
			if (repository.Name == DefaultRepositoryName)
			{
				...
				string repositoryConfigFile = SystemInfo.GetAppSetting("X3Platform.Logging.ConfigFile");
				if (repositoryConfigFile != null && repositoryConfigFile.Length > 0)
				{
					...
					{
						LogLog.Warn("DefaultRepositorySelector: Exception getting ApplicationBaseDirectory. appSettings X3Platform.Logging.Config path ["+repositoryConfigFile+"] will be treated as an absolute URI", ex);
					}
					...
					{
						LogLog.Error("DefaultRepositorySelector: Exception while parsing X3Platform.Logging.Config file path ["+repositoryConfigFile+"]", ex);
					}
		...
		private void LoadPlugins(Assembly assembly, ILoggerRepository repository)
		{
			...
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(X3Platform.Logging.Plugin.IPluginFactory configAttr in configAttributes)
				{
					try
					{
		...
		private const string DefaultRepositoryName = "X3Platform.Logging-default-repository";
