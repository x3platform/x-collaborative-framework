...
namespace X3Platform.Logging.Core
{
    ...
    public class DefaultRepositorySelector : IRepositorySelector
    {
        ...
        public DefaultRepositorySelector(Type defaultRepositoryType)
        {
            ...
            {
                throw X3Platform.Logging.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
            }
        ...
        private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
        {
            ...
            if (repository.Name == DefaultRepositoryName)
            {
                ...
                string repositoryConfigFile = SystemInfo.GetAppSetting("X3Platform.Logging.Config");
                if (repositoryConfigFile != null && repositoryConfigFile.Length > 0)
                {
                    ...
                    {
                        LogLog.Warn(declaringType, "Exception getting ApplicationBaseDirectory. appSettings X3Platform.Logging.Config path [" + repositoryConfigFile + "] will be treated as an absolute URI", ex);
                    }
...
#if NET_2_0 || MONO_2_0
				    Boolean.TryParse(SystemInfo.GetAppSetting("Watch"), out watchRepositoryConfigFile);
#else
                    {
                        string watch = SystemInfo.GetAppSetting("Watch");
                    ...
                    if (watchRepositoryConfigFile)
                    {
                        ...
                        {
                            LogLog.Error(declaringType, "DefaultRepositorySelector: Exception while parsing X3Platform.Logging.Config file physical path [" + repositoryConfigFilePath + "]", ex);
                        }
                        try
                        {
                    ...
                    else
                    {
                        ...
                        {
                            LogLog.Error(declaringType, "Exception while parsing X3Platform.Logging.Config file path [" + repositoryConfigFile + "]", ex);
                        }
