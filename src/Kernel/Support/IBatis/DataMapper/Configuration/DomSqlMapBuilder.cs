...
using X3Platform.IBatis.DataMapper.Configuration.Cache;
using X3Platform.IBatis.DataMapper.Configuration.Cache.Fifo;
using X3Platform.IBatis.DataMapper.Configuration.Cache.Lru;
using X3Platform.IBatis.DataMapper.Configuration.Cache.Memory;
using X3Platform.IBatis.DataMapper.Configuration.ParameterMapping;
using X3Platform.IBatis.DataMapper.Configuration.ResultMapping;
using X3Platform.IBatis.DataMapper.Configuration.Serializers;
using X3Platform.IBatis.DataMapper.Configuration.Sql;
using X3Platform.IBatis.DataMapper.Configuration.Sql.Dynamic;
using X3Platform.IBatis.DataMapper.Configuration.Sql.Dynamic.Elements;
using X3Platform.IBatis.DataMapper.Configuration.Sql.SimpleDynamic;
using X3Platform.IBatis.DataMapper.Configuration.Sql.Static;
using X3Platform.IBatis.DataMapper.Configuration.Statements;
using X3Platform.IBatis.DataMapper.MappedStatements;
using X3Platform.IBatis.DataMapper.MappedStatements.ArgumentStrategy;
using X3Platform.IBatis.DataMapper.MappedStatements.PropertyStrategy;
...
namespace X3Platform.IBatis.DataMapper.Configuration
{
    /// <summary>
    ...
    public class DomSqlMapBuilder
    {
        ...
        private static readonly IInternalLog _logger = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
            ...
            bool useConfigFileWatcher, bool isCallFromDao)
        {
            ...
            {
                throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException(_configScope.ErrorContext.ToString(), e);
            }
        }
        ...
        private void ValidateSchema(XmlNode section, string schemaFileName)
        {
            ...
            try
            {
                ...
                if (xsdFile == null)
                {
                    // TODO: avoid using hard-coded value "X3Platform.IBatis.DataMapper"
                    throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException("Unable to locate embedded resource [X3Platform.IBatis.DataMapper." + schemaFileName + "]. If you are building from source, verfiy the file is marked as an embedded resource.");
                }
                ...
                {
                    throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException("Invalid SqlMap.config document. cause :" + _configScope.ErrorContext.Resource);
                }
            }
            finally
            {
        ...
        private void Initialize()
        {
            ...
            if (nodeDataSource == null)
            {
                if (_configScope.IsCallFromDao == false)
                {
                    throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException("There's no dataSource tag in SqlMap.config.");
                }
        ...
        private void ProcessSqlStatement(IStatement statement)
        {
            ...
            if (statement.ExtendStatement.Length > 0)
            {
                ...
                {
                    throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException("Unable to find extend statement named '" + statement.ExtendStatement + "' on statement '" + statement.Id + "'.'");
                }
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else if (child.Name == "include")
                {
                    ...
                    if (includeNode == null)
                    {
                        ...
                        if (includeNode == null)
                        {
                            throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException("Could not find SQL tag to include with refid '" + refid + "'");
                        }
                    }
            ...
            if (_configScope.SqlMapper.ParameterMaps.Contains(id) == false)
            {
                ...
                if (parameterMap.ExtendMap.Length > 0)
                {
                    ...
                    if (_configScope.SqlMapper.ParameterMaps.Contains(parameterMap.ExtendMap) == false)
                    {
                        ...
                        {
                            throw new X3Platform.IBatis.Common.Exceptions.ConfigurationException("In mapping file '" + _configScope.SqlMapNamespace + "' the parameterMap '" + parameterMap.Id + "' can not resolve extends attribute '" + parameterMap.ExtendMap + "'");
                        }
                    }
                    else
                    {
            ...
            if (_configScope.SqlMapper.ResultMaps.Contains(id) == false)
            {
                ...
                if (resultMap.ExtendMap != null && resultMap.ExtendMap.Length > 0)
                {
                    ...
                    if (_configScope.SqlMapper.ResultMaps.Contains(resultMap.ExtendMap) == false)
                    {
                        ...
                        {
                            throw new System.Configuration.ConfigurationException("In mapping file '" + _configScope.SqlMapNamespace + "' the resultMap '" + resultMap.Id + "' can not resolve extends attribute '" + resultMap.ExtendMap + "'");
                        }
                    }
                    else
                    {
        ...
        {
            return Assembly.GetExecutingAssembly().GetManifestResourceStream("X3Platform.IBatis.DataMapper." + schemaResourceKey);
        }
