x.register('main.forum.my.thread.list');

main.forum.my.thread.list = {
  ...
  */
  filter: function()
  {
    ...
    if(main.forum.my.thread.list.searchNodeName != "")
    {
      var searchNodeName = main.forum.my.thread.list.searchNodeName;
  ...
  */
  getObjectsView: function(list, maxCount)
  {
    ...
    x.each(list, function(index, node)
    {
      ...
      if($('#isAdminToken').val().toLowerCase() == 'true')
      {
        outString += '<td><a href="javascript:main.forum.my.thread.list.confirmDelete(\'' + node.id + '\',\'' + node.accountId + '\',\'' + node.sign + '\');" title="删除" ><i class="fa fa-trash" ></i></a></td>';
      }
      outString += '</tr>';
  ...
  */
  ...
  {
    var paging = main.forum.my.thread.list.paging;
    ...
    x.net.xhr('/api/forum.thread.query.aspx', outString, {
      callback: function(response)
      {
        ...
        var containerHtml = main.forum.my.thread.list.getObjectsView(list, paging.pageSize);

        $('#window-main-table-container').html(containerHtml);

        var footerHtml = paging.tryParseMenu('javascript:main.forum.my.thread.list.getPaging({0});');
  ...
  */
  confirmDelete: function(id, accountId, sign)
  {
    if(confirm("确定永久删除该帖子及相关信息吗？"))
    {
      ...
      $.post(main.forum.my.thread.list.url, options, this.confirmDelete_callback);
    }
  },
  ...
  */
  load: function()
  {
    main.forum.my.thread.list.filter();
...
$(document).ready(main.forum.my.thread.list.load);
