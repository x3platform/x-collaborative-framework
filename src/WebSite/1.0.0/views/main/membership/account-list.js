  ...
  */
  save: function()
  {
    if(main.membership.account.list.checkObject())
    {
      ...
      outString += '<password><![CDATA[' + ($("#password").val() === '' ? '888888' : $("#password").val()) + ']]></password>';
  ...
  setPassword: function(password)
  {
    if(confirm('确定将此帐号密码重置为[' + password + ']?'))
    {
      ...
      $.post(main.membership.account.list.url,
             options,
             main.membership.account.list.setPassword_callback);
    }
  },

  setPassword_callback: function(response)
  {
