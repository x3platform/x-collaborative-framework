    ...
    var main = {
        ...
        */
        getObjectsView: function(list, maxCount)
        {
            ...
            outString += '<tr>';
            outString += '<th >名称</th>';
            outString += '<th style="width:80px">接受</th>';
            outString += '<th style="width:60px">' + i18n.strings.btn_delete + '</th>';
            ...
            {
                outString += '<tr>';
                outString += '<td><a href="/admins/friends/detail/' + node.friendAccountId + '" >' + node.friendAccountName + '</a></td>';
                outString += '<td>' + (node.status == 1 ? '已接受' : '<a href="javascript:main.accept(\'' + node.friendAccountId + '\');">接受</a>') + '</td>';
                outString += '<td><a href="javascript:main.confirmDelete(\'' + node.friendAccountId + '\',\'' + node.accountId + '\');">' + i18n.strings.btn_delete + '</a></td>';
                outString += '</tr>';
        ...
        */
        getPaging: function(currentPage)
        {
            ...
            x.net.xhr('/api/membership.accountFriend.queryAcceptList.aspx', outString, {
                waitingType: 'mini',
         ...
         * 删除对象
         */
        accept: function(friendAccountId)
        {
            x.net.xhr('/api/membership.accountFriend.accept.aspx?friendAccountId=' + friendAccountId, {
                callback: function(response)
                {
