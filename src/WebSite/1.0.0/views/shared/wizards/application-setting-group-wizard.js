 ...
 * 应用参数分组向导
 */
x.ui.wizards.newApplicationSettingGroupWizard = function(name, targetViewName, targetValueName, options)
{
  var wizard = {
    ...
    save: function()
    {
      this.returnValue = '{"text":"' + $(document.getElementById(this.name + '-wizardApplicationSettingGroupText')).val() + '","value":"' + $(document.getElementById(this.name + '-wizardApplicationSettingGroupValue')).val() + '"}';
    ...
    */
    ...
    {
      $('#' + this.name + '-wizardApplicationSettingGroupText').val(text);
      $('#' + this.name + '-wizardApplicationSettingGroupValue').val(value);
    ...
    create: function()
    {
      ...
      outString += '<div class="winodw-wizard-toolbar-item" style="width:200px" ><span>应用参数分组向导</span></div>';
      ...
      outString += '<label class="winodw-wizard-result-item-text" >分组名称</label> ';
      outString += '<input id="' + this.name + '-wizardApplicationSettingGroupText" name="' + this.name + '-wizardApplicationSettingGroupText" type="text" value="" class="winodw-wizard-result-item-input form-control input-sm" style="width:160px" /> ';
      outString += '<input id="' + this.name + '-wizardApplicationSettingGroupValue" name="' + this.name + '-wizardApplicationSettingGroupValue" type="hidden" value="" />';
    ...
    load: function()
    {
      ...
      if(this.localStorage !== null)
      {
        ...
        $('#' + this.name + '-wizardApplicationSettingGroupText').val(node.text);
        $('#' + this.name + '-wizardApplicationSettingGroupValue').val(node.value);
      }
    }
  };
...
/*#region 函数:x.ui.wizards.getApplicationSettingGroupWizard(options)*/
/*
* 快速创建单例
*/
x.ui.wizards.getApplicationSettingGroupWizard = function(options)
{
  var name = x.getFriendlyName(location.pathname + '-' + options.applicationId + '-' + options.targetViewName + '-' + options.targetValueName + '-application-wizard');
  ...
  if(typeof (window[name]) === 'undefined')
  {
    ...
    // 初始化向导
    var wizard = x.ui.wizards.newApplicationSettingGroupWizard(name, options.targetViewName, options.targetValueName, options);
