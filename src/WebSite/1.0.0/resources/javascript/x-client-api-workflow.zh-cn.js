    ...
    download: function(options)
    {
        ...
        x.net.xhr(options.url, outString, {
            ...
            callback: function(response)
            {
                ...
                outString += '<td style="width:40px;" >节点</td>';
                outString += '<td style="width:100px;" >节点名</td>';
                outString += '<td >执行人</td>';
                outString += '<td style="width:80px;" >下一步骤</td>';
                outString += '<td style="width:60px;" >执行方式</td>';
                outString += '<td style="width:40px;" >时限</td>';
                outString += '<td style="width:40px;" >编辑</td>';
                outString += '<td style="width:40px;" >上移</td>';
                outString += '<td style="width:40px;" >下移</td>';
        ...
        nodes.each(function(index, node)
        {
            ...
            else if(nodeIndex < nodes.length)
            {
                ...
                else
                {
                    ...
                    exits.each(function(exit, exitIndex)
                    {
                        if(exitIndex > 0)
                        {
                            exitToNodeIds[exit.toNodeId] = exit.toNodeId;
        ...
        getStartRoleWizard: function(accountId)
        {
            ...
            x.ui.wizards.getWizard('workflow-startRole', {
                ...
                create: function()
                {
                    ...
                    outString += '<div class="winodw-wizard-result-item" ><span class="winodw-wizard-result-item-text" style="width:40px;" >角色</span></div>';
                    outString += '<div class="winodw-wizard-result-item" ><input id="workflow-startRole-wizardText" name="wizardText" type="text" value="' + $('#workflow-template-startRoleName').val() + '" class="winodw-wizard-result-item-input" style="width:100px;" /><input id="workflow-startRole-wizardValue" name="wizardValue" type="hidden" value="' + $('#workflow-template-startRoleId').val() + '" /></div>';
                ...
                callback: function(response)
                {
                    ...
                    x.each(list, function(index, node)
                    {
                        if (node.value !== '')
                        {
                            ...
                            outString += '<td>';
                            outString += '<a href="javascript:x.form.query(\'workflow-startRole-wizardValue\').val(\'' + node.id + '\');x.form.query(\'workflow-startRole-wizardText\').val(\'' + node.name + '\'); void(0);" >' + node.name + '</a></li>';
...
x.workflow.node = {
    ...
    sync: function()
    {
        ...
        for(var i = 0;i < nodes.length;i++)
        {
            ...
            if(i === 0)
            {
                ...
                {
                    node.childNodes[6].innerHTML = '<a href="javascript:x.workflow.node.edit(' + index + ')">编辑</a>';
                }
                else
                {
                    node.childNodes[6].innerHTML = '<span class="gray-text" >编辑</span>';
                }
                node.childNodes[7].innerHTML = '<span class="gray-text" >上移</span>';
                node.childNodes[8].innerHTML = '<span class="gray-text" >下移</span>';
                node.childNodes[9].innerHTML = '<span class="gray-text" >删除</span>';
            }
            else if(($(document.getElementById('workflow-administrator')).val() !== '1') && $(document.getElementById('workflow-editableTemplate')).val() === '0')
            {
                // 禁止编辑模板
                node.childNodes[6].innerHTML = '<span class="gray-text" >编辑</span>';
                node.childNodes[7].innerHTML = '<span class="gray-text" >上移</span>';
            ...
            else
            {
                ...
                {
                    node.childNodes[7].innerHTML = '<a href="javascript:x.workflow.node.move(' + index + ',\'up\')">上移</a>';
                    node.childNodes[8].innerHTML = '<a href="javascript:x.workflow.node.move(' + index + ',\'down\');">下移</a>';
                }
                else
                {
                    node.childNodes[7].innerHTML = '<span class="gray-text" >上移</span>';
                ...
                {
                    node.childNodes[9].innerHTML = '<a href="javascript:x.workflow.node.remove(' + index + ');">删除</a>';
                }
                else
                {
                    node.childNodes[9].innerHTML = '<span class="gray-text" >删除</span>';
    ...
    */
    edit: function(index)
    {
        ...
        outString += '<div class="winodw-wizard-toolbar" >';
        outString += '<div class="winodw-wizard-toolbar-close">';
        outString += '<a href="javascript:x.workflow.node.maskWrapper.close();" class="button-text" >关闭</a>';
        outString += '</div>';
        outString += '<div class="float-left">';
        ...
        outString += '<div class="x-ui-pkg-tabs-wrapper" style="height: 320px;">';
        outString += '<div id="workflow-node-tabs-menu-wrapper" class="x-ui-pkg-tabs-menu-wrapper" >';
        outString += '<ul class="x-ui-pkg-tabs-menu" >';
        ...
        outString += '<input id="workflow-node-name" name="workflow-node-name" type="text" class="input-normal" style="width:200px;" value="' + ((typeof (node.name) === 'undefined') ? '' : node.name) + '" />';
        outString += '</td>';
        outString += '</tr>';
        ...
        outString += '<td colspan="3" class="table-body-input">';
        outString += '<textarea id="workflow-node-remark" name="workflow-node-remark" type="text" class="textarea-normal" style="width:520px; height:40px;" >' + x.isUndefined(node.remark, '', x.encoding.html.decode(node.remark)) + '</textarea>';
        outString += '</tr>';

        outString += '<tr>';
        outString += '<td class="table-body-text">执行方式</td>';
        outString += '<td class="table-body-input">';
        outString += '<input id="workflow-node-actorMethod" name="workflow-node-actorMethod" feature="combobox" data="' + ((document.getElementById('workflow-nodeActorMethods') === null) ? settings.workflowNodeActorMethods : $('#workflow-nodeActorMethods').val()) + '" topOffset="-1" selectedText="' + ((typeof (node.actorMethod) === 'undefined') ? '审核' : node.actorMethod) + '" value="' + ((typeof (node.actorMethod) === 'undefined') ? '并审' : node.actorMethod) + '" class="input-normal" style="width:180px" />';
        outString += '</td>';
        outString += '<td class="table-body-text" style="width:120px;" >处理时限(小时)</td>';
        outString += '<td class="table-body-input"><input id="workflow-node-timelimit" name="workflow-node-timelimit" feature="number" type="text" class="input-normal" style="width:120px;" value="' + ((typeof (node.timelimit) === 'undefined') ? '24' : node.timelimit) + '" /></td>';
        outString += '</tr>';
        ...
        outString += '<td class="table-body-input"><input id="workflow-node-editor" name="workflow-node-editor" type="text" class="input-normal" style="width:95%;" value="' + ((typeof (node.editor) === 'undefined') ? '' : node.editor) + '" /></td>';
        outString += '</tr>';

        outString += '<tr >';
        outString += '<td class="table-body-text">处理器</td>';
        outString += '<td class="table-body-input"><input id="workflow-node-handler" name="workflow-node-handler" type="text" class="input-normal" style="width:95%;" value="' + ((typeof (node.handler) === 'undefined') ? '' : node.handler) + '" /></td>';
        outString += '</tr>';

        outString += '<tr >';
        outString += '<td class="table-body-text" >自定义待办提醒</td>';
        outString += '<td class="table-body-input vertical-middle">';
        outString += '<input id="workflow-node-policy-settings-sendAlertTaskFormat" name="workflow-node-settings$sendAlertTaskFormat" type="text" value="' + ((typeof (node.policy.setting.sendAlertTaskFormat) === 'undefined') ? '' : node.policy.setting.sendAlertTaskFormat) + '" class="input-normal" style="width:360px" />';
        outString += '</td>';
        outString += '</tr>';
        ...
        outString += '<a href="javascript:x.wizards.getContactsWizard({targetViewName:\'workflow-node-commissionActorDescription\',targetValueName:\'workflow-node-commissionActors\', contactTypeText:\'account\'});" >编辑</a>';
        outString += '</div> ';
        outString += '</td>';
        outString += '</tr>';
        outString += '<tr >';
        ...
        outString += '<div class="float-right button-2font-wrapper" ><a id="btnCancel" href="javascript:void(0);" class="button-text" >取消</a></div> ';
        outString += '<div class="float-right button-2font-wrapper" style="margin-right:10px;" ><a id="btnOkey" href="javascript:void(0);" class="button-text" >保存</a></div> ';
        outString += '<div class="clear"></div>';
        outString += '</div>';
    ...
    */
    setNodeObject: function(node, index)
    {
        ...
        {
            node.switcher.exits.each(function(exit)
            {
                exits += '{';
    ...
    */
    showActorWizard: function()
    {
        ...
        // 人员选择向导的人员选择范围
        var contactTypeText = document.getElementById('workflow-node-contactTypeText') === null ? 'all' : $(document.getElementById('workflow-node-contactTypeText')).val();

        x.wizards.getContactsWizard({
            ...
            save_callback: function(response)
            {
                ...
                // site.v1.apps.pages.contacts.home.localStorage = response;

                $(document.getElementById('workflow-node-actorDescription')).val(resultView);
                $(document.getElementById('workflow-node-actorScope')).val(resultValue);
            },
...
x.workflow.switcherExit = {
    ...
    edit: function(index)
    {
        ...
        outString += '<div class="winodw-wizard-toolbar" >';
        outString += '<div class="winodw-wizard-toolbar-close">';
        outString += '<a href="javascript:x.workflow.switcherExit.maskWrapper.close();" class="button-text" >关闭</a>';
        outString += '</div>';
        outString += '<div class="float-left">';
        ...
        outString += '<td ><input id="workflow-node-switcher-exit-condition-join" type="text" feature="combobox" topOffset="-1" url="/api/application.setting.getCombobox.aspx" comboboxWhereClause=" ApplicationSettingGroupId IN ( SELECT Id FROM tb_Application_SettingGroup WHERE Name = ##应用管理_协同平台_工作流管理_分支出口条件管理_链接方式## ) " class="input-normal" style="width:60px" /></td>';
        outString += '<td ><input id="workflow-node-switcher-exit-condition-leftBracket" type="text" feature="combobox" topOffset="-1" url="/api/application.setting.getCombobox.aspx" comboboxIconHidden="1" comboboxWhereClause=" ApplicationSettingGroupId IN ( SELECT Id FROM tb_Application_SettingGroup WHERE Name = ##应用管理_协同平台_工作流管理_分支出口条件管理_左侧括号## ) " class="input-normal" style="width:30px" /></td>';
        outString += '<td >';
        outString += '<input id="workflow-node-switcher-exit-condition-expression1" type="hidden"/> ';
        outString += '<input id="workflow-node-switcher-exit-condition-expression1_view" type="text" class="input-normal" readonly="readonly" style="width:150px" /> ';
        outString += '<a href="javascript:x.workflow.switcherExit.expression.edit(\'workflow-node-switcher-exit-condition-expression1\',\'left\');" >编辑</a>';
        outString += '</td>';
        outString += '<td ><input id="workflow-node-switcher-exit-condition-compare" type="text" feature="combobox" topOffset="-1" url="/api/application.setting.getCombobox.aspx" comboboxWhereClause=" ApplicationSettingGroupId IN ( SELECT Id FROM tb_Application_SettingGroup WHERE Name = ##应用管理_协同平台_工作流管理_分支出口条件管理_判断方式## ) " class="input-normal" style="width:80px" /></td>';
        outString += '<td >';
        ...
        outString += '<input id="workflow-node-switcher-exit-condition-expression2_view" type="text" class="input-normal" style="width:150px" /> ';
        ...
        outString += '</td>';
        outString += '<td ><input id="workflow-node-switcher-exit-condition-rightBracket" type="text" feature="combobox" topOffset="-1" url="/api/application.setting.getCombobox.aspx" comboboxIconHidden="1" comboboxWhereClause=" ApplicationSettingGroupId IN ( SELECT Id FROM tb_Application_SettingGroup WHERE Name = ##应用管理_协同平台_工作流管理_分支出口条件管理_右侧括号## ) " class="input-normal" style="width:30px" /></td>';
        outString += '<td >';
        outString += '<a href="javascript:x.workflow.switcherExit.condition.setEditConditionObject();" >确定</a>';
        outString += '</div>';
        outString += '</td>';
        outString += '</tr>';
        outString += '</table>';
        ...
        outString += '<div style="border-top:1px solid #ccc;padding:10px 20px 10px 0;" >';
        outString += '<div class="float-right button-2font-wrapper" ><a id="btnCancel" href="javascript:x.workflow.switcherExit.maskWrapper.close();" class="button-text" >取消</a></div> ';
        outString += '<div class="float-right button-2font-wrapper" style="margin-right:10px;" ><a id="btnOkey" href="javascript:x.workflow.switcherExit.save();" class="button-text" >保存</a></div> ';
        outString += '<div class="clear"></div>';
        outString += '</div>';
        ...
        // 加载遮罩和页面内容
        x.mask.getWindow(outString, x.workflow.switcherExit.maskWrapper);
        // 设置条件表格
    ...
    setConditionTable: function(exit)
    {
        ...
        for (var i = 0; i < list.length; i++)
        {
            ...
            outString += '</td>';
            outString += '<td><a href="javascript:x.workflow.switcherExit.condition.edit(' + (i + 1) + ')">编辑</a></td>';
            outString += '<td><a href="javascript:x.workflow.switcherExit.condition.remove(' + (i + 1) + ')">删除</a></td>';
            outString += '</tr>';
        }
    ...
    condition: {
        ...
        reset: function()
        {
            ...
            document.getElementById('workflow-node-switcher-exit-condition-join').value = 'AND';
            document.getElementById('workflow-node-switcher-exit-condition-join_view').value = '并且';
            document.getElementById('workflow-node-switcher-exit-condition-leftBracket').value = '(';
            document.getElementById('workflow-node-switcher-exit-condition-leftBracket_view').value = '(';
            document.getElementById('workflow-node-switcher-exit-condition-expression1').value = '';
            document.getElementById('workflow-node-switcher-exit-condition-expression1_view').value = '';
            document.getElementById('workflow-node-switcher-exit-condition-compare_view').value = '等于';
            document.getElementById('workflow-node-switcher-exit-condition-compare').value = '=';
            document.getElementById('workflow-node-switcher-exit-condition-expression2').value = '';
            document.getElementById('workflow-node-switcher-exit-condition-expression2_view').value = '';
            document.getElementById('workflow-node-switcher-exit-condition-handmade').value = 0;
            document.getElementById('workflow-node-switcher-exit-condition-rightBracket').value = ')';
            document.getElementById('workflow-node-switcher-exit-condition-rightBracket_view').value = ')';
        },
        /*#endregion*/
        ...
        */
        sync: function()
        {
            ...
            for (var i = 0; i < list.length; i++)
            {
                ...
                row.childNodes[2].innerHTML = '<a href="javascript:x.workflow.switcherExit.condition.edit(' + index + ')">编辑</a>';
                row.childNodes[3].innerHTML = '<a href="javascript:x.workflow.switcherExit.condition.remove(' + index + ')">删除</a>';
            }
        },
        /*#endregion*/
        ...
        edit: function(index)
        {
            ...
            document.getElementById(editPrefix + '$join_view').value = document.getElementById(idPrefix + '$joinText').value;
            document.getElementById(editPrefix + '$leftBracket').value = document.getElementById(idPrefix + '$leftBracket').value;
            document.getElementById(editPrefix + '$leftBracket_view').value = document.getElementById(idPrefix + '$leftBracket').value;
            document.getElementById(editPrefix + '$expression1').value = document.getElementById(idPrefix + '$expression1').value;
            document.getElementById(editPrefix + '$expression1_view').value = document.getElementById(idPrefix + '$expression1Text').value;
            document.getElementById(editPrefix + '$compare').value = document.getElementById(idPrefix + '$compare').value;
            document.getElementById(editPrefix + '$compare_view').value = document.getElementById(idPrefix + '$compareText').value;
            document.getElementById(editPrefix + '$expression2').value = document.getElementById(idPrefix + '$expression2').value;
            document.getElementById(editPrefix + '$expression2_view').value = document.getElementById(idPrefix + '$expression2Text').value;
            ...
            document.getElementById(editPrefix + '$rightBracket_view').value = document.getElementById(idPrefix + '$rightBracket').value;

            x.form.query(editPrefix + 'expression1_view').css({ 'text-decoration': 'underline', 'font-weight': 'bold' });

            if (document.getElementById(idPrefix + '$handmade').value == '0')
            {
                x.form.query(editPrefix + 'expression2_view').css({ 'text-decoration': 'underline', 'font-weight': 'bold' });
            }
            else
            {
                x.form.query(editPrefix + 'expression2_view').css({ 'text-decoration': 'none', 'font-weight': 'normal' });
            }
        },
        /*#endregion*/
        ...
        setEditConditionObject: function()
        {
            ...
            if (index === 0)
            {
                ...
                outString += '<input id="' + idPrefix + '$rightBracket" type="hidden" value="' + condition.rightBracket + '" />';
                outString += '</td>';
                outString += '<td><a href="javascript:x.workflow.switcherExit.condition.edit(' + index + ')">编辑</a></td>';
                outString += '<td><a href="javascript:x.workflow.switcherExit.condition.remove(' + index + ')">删除</a></td>';
                outString += '</tr>';
        ...
        */
        ...
        {
            var join = document.getElementById('workflow-node-switcher-exit-condition-join').value;
            var leftBracket = document.getElementById('workflow-node-switcher-exit-condition-leftBracket_view').value;
            var expression1 = document.getElementById('workflow-node-switcher-exit-condition-expression1_view').value;
            var compare = document.getElementById('workflow-node-switcher-exit-condition-compare_view').value;
            var expression2 = document.getElementById('workflow-node-switcher-exit-condition-expression2_view').value;
            var handmade = document.getElementById('workflow-node-switcher-exit-condition-handmade');
            var rightBracket = document.getElementById('workflow-node-switcher-exit-condition-rightBracket_view').value;

            var friendlyText = leftBracket + '' + expression1 + ' ' + compare + ' ' + expression2 + '' + rightBracket;

            if (handmade === '1')
            {
                document.getElementById('workflow-node-switcher-exit-condition-expression2').value = document.getElementById('workflow-node-switcher-exit-condition-expression2_view').value
            }
            ...
            outString += '{';
            outString += '"join":"' + document.getElementById('workflow-node-switcher-exit-condition-join').value + '", ';
            outString += '"joinText":"' + document.getElementById('workflow-node-switcher-exit-condition-join_view').value + '", ';
            outString += '"leftBracket":"' + document.getElementById('workflow-node-switcher-exit-condition-leftBracket').value + '", ';
            outString += '"expression1":"' + document.getElementById('workflow-node-switcher-exit-condition-expression1').value + '", ';
            outString += '"expression1Text":"' + document.getElementById('workflow-node-switcher-exit-condition-expression1_view').value + '", ';
            outString += '"compare":"' + document.getElementById('workflow-node-switcher-exit-condition-compare').value + '", ';
            outString += '"compareText":"' + document.getElementById('workflow-node-switcher-exit-condition-compare_view').value + '", ';
            outString += '"expression2":"' + document.getElementById('workflow-node-switcher-exit-condition-expression2').value + '", ';
            outString += '"expression2Text":"' + document.getElementById('workflow-node-switcher-exit-condition-expression2_view').value + '", ';
            outString += '"handmade":"' + (document.getElementById('workflow-node-switcher-exit-condition-handmade').checked ? 1 : 0) + '", ';
            outString += '"rightBracket":"' + document.getElementById('workflow-node-switcher-exit-condition-rightBracket').value + '", ';
    ...
    expression: {
        ...
        */
        getTreeView: function(entitySchemaId, expressionType)
        {
            ...
            var tree = x.tree.newTreeView('x.workflow.switcherExit.expression.tree');
            ...
            if (list.length > 0)
            {
                tree.add('00000', '0', '表单内置信息', 'javascript:void(0);');
            ...
            document.getElementById('workflow-switcherExit$expression$wizardTreeViewContainer').innerHTML = tree;
        },
        /*#endregion*/
        ...
        {
            x.form.query('workflow-switcherExit$expression$wizardExpressionText').val(text);
            x.form.query('workflow-switcherExit$expression$wizardExpressionValue').val(value);
        },
        /*#endregion*/
        ...
        edit: function(expressionName, expressionType)
        {
            ...
            outString += '<div class="winodw-wizard-toolbar" >';
            outString += '<div class="winodw-wizard-toolbar-close">';
            outString += '<a href="javascript:x.workflow.switcherExit.expression.maskWrapper.close();" class="button-text" >关闭</a>';
            outString += '</div>';
            outString += '<div class="float-left">';
            ...
            outString += '<div class="winodw-wizard-result-container" >';
            outString += '<div class="winodw-wizard-result-item"><span class="winodw-wizard-result-item-text" >表达式</span></div>';
            outString += '<div class="winodw-wizard-result-item"><input id="workflow-switcherExit$expression$wizardExpressionText" name="wizardExpressionText" type="text" value="" class="winodw-wizard-result-item-input" /><input id="workflow-switcherExit$expression$wizardExpressionValue" name="wizardExpressionValue" type="hidden" value="" /></div>';
            outString += '<div class="winodw-wizard-result-item"><div class="button-2font-wrapper" style="margin-right:10px;" ><a href="javascript:x.workflow.switcherExit.expression.save(\'' + expressionName + '\');" class="button-text" >确定</a></div></div>';
            outString += '<div class="clear"></div>';
            outString += '</div>';
            ...
            // 加载遮罩和页面内容
            x.mask.getWindow(outString, x.workflow.switcherExit.expression.maskWrapper);

            // 初始化数据
            x.form.query('workflow-switcherExit$expression$wizardExpressionText').val(x.form.query(expressionName + '_view').val());
            x.form.query('workflow-switcherExit$expression$wizardExpressionValue').val(x.form.query(expressionName).val());

            x.net.xhr('/api/kernel.entities.schema.findOneByEntityClassName.aspx?entityClassName=' + encodeURIComponent(x.form.query('workflow-template$entityClass').val()), {
                callback: function(response)
                {
        ...
        save: function(expressionName)
        {
            ...
            x.form.query(expressionName + '_view').val(x.form.query('workflow-switcherExit$expression$wizardExpressionText').val());
            x.form.query(expressionName).val(x.form.query('workflow-switcherExit$expression$wizardExpressionValue').val());
            x.form.query(expressionName + '_view').css({ 'text-decoration': 'underline', 'font-weight': 'bold' });
            x.form.query('workflow-node-switcher-exit-condition-handmade').val(0);
        }
        /*#endregion*/
    }
};