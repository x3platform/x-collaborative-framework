...
(function(global, factory) 
{
    ...
    */
    ...
    {
        var type = x.type(node);
    
        if (type == 'array' || type == 'function') { return ''; }
        ...
        */
        defaults: {
            ...
            getWaitingWindow: function(options)
            {
                ...
                options = x.ext({
                    type: 'default',                                      // 窗口类型
                    text: i18n.strings.msg_net_waiting_commit_tip_text    // 提示信息
                }, options || {});
        ...
        getWaitingWindow: function(options)
        {
            return x.net.defaults.getWaitingWindow(options);
        },
        /*#endregion*/
        ...
        xhr: function()
        {
            ...
            x.net.ajax({
                ...
                success: function(response)
                {
                    ...
                    {
                        // 捕获处理异常
                        if (!options.catchException(response, options.outputException))
                        {
                            var result = x.toJSON(response);
                ...
                error: function(XMLHttpRequest, textStatus, errorThrown)
                {
                    ...
                    else
                    {
                        ...
                        {
                            x.msg(i18n.strings.msg_net_error_401);
                        }
                        else if (XMLHttpRequest.status == 404)
                        {
                            x.msg(i18n.strings.msg_net_error_404);
                        }
                        else if (XMLHttpRequest.status == 500)
                        {
                            x.msg(i18n.strings.msg_net_error_500);
                        }
                        else if (XMLHttpRequest.status != 0)
                        {
                            x.debug.error(i18n.strings.msg_net_error_unkown.format(XMLHttpRequest.status + (XMLHttpRequest.statusText != '' ? (' ' + XMLHttpRequest.statusText) : '')));
                        }
                    }
                }
            });
